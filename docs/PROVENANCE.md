...
- `Formalizations/R0/Main.lean`：将已有 R0 完整证明作为首个问题形式化项目接入，提供下界、达到最小规模、最小元及等式的应用入口。原 R0 源码和接口未改变；该接入不代表新的数学结果。应用入口由 `CheckFormalizations.lean` 与统一 `Audit.lean` 检查。
