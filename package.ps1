...
foreach ($icGate in @('build','consumer_examples','unfolded_R0_statements','axiom_audit','module_coverage','forbidden_construct_scan')) {
...
foreach ($icSource in (Get-ChildItem -LiteralPath (Join-Path $PSScriptRoot 'InfinitaryCombinatorics'),(Join-Path $PSScriptRoot 'R0') -Filter '*.lean' -Recurse)) {
